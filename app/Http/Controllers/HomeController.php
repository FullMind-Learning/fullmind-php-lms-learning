...
//        dd(Auth::user()->unreadNotifications[0]->data['actionURL'] ?? '');
        ...
        try {
            ...
            if ($user->role_id == 1) {
                ...
                $coursesEarnings = $query2->groupBy('month_number', 'year', 'month')
                    ->whereYear('created_at', Carbon::now()->year)
                    ->get();

                $query3 = CourseEnrolled::select(
                    DB::raw('MONTHNAME(created_at) as month'),
                    DB::raw('YEAR(created_at) as year'),
                ...
                $courses_enrolle = $query3->groupBy('year', 'month', 'day')->get();
            }
    ...
    public function getDashboardData()
    {
        try {
            ...
            if ($user->role_id == 2) {
                ...
                $query5 = CourseEnrolled::select(
                    DB::raw('sum(purchase_price) as totalSell'),
                    DB::raw("DATE_FORMAT(created_at,'%m') as months")
                );
                if (isModuleActive('Organization') && Auth::user()->isOrganization()) {
                    $query5->whereHas('course.user', function ($q) {
                        $q->where('organization_id', Auth::id());
                        $q->orWhere('user_id', Auth::id());
                    });
                }

                $month = $query5->groupBy('months')->get();
